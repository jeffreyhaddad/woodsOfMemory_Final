...
public class MapSystem : MonoBehaviour
{
    ...
    void BuildMinimapUI()
    {
        ...
        Sprite    knob   = MakeCircleSprite(128);
    ...
    /// <summary>Generates a filled circle sprite at runtime, replacing the deprecated UI/Skin/Knob.psd.</summary>
    static Sprite MakeCircleSprite(int size)
    {
        ...
        for (int y = 0; y < size; y++)
        {
            for (int x = 0; x < size; x++)
            {
                ...
                float alpha = Mathf.Clamp01(r - dist); // anti-aliased edge
